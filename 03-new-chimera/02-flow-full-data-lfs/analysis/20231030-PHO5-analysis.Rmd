...
my_plot_region_effect_twovar_line_par <- function(regions){
  ...
  p <- dat %>% 
    ggplot(aes(x = host, y = diff, label = bg)) +
    geom_point(aes(color = grp, shape = sh), size = 2, alpha = 0.8,
...
```{r}
my_plot_region_effect_twovar_line_par(c(1,2,3))
ggsave("../img/20240214-region-swap-effect-1to3-on-4.png")
```
...
## Region 4 splits
We have so far focused on the main set with the 5 region design. In the scatter plot above, we see that there is a subset of chimeras in between the P2ID:Sc and P2ID:Cg ones. They are interesting in that their A_pho2∆/A_PHO2 ratios are intermediate.
![scatter](../img/20231220-all-chimera-scatter-color-by-P2ID.png)
Below, we will zoom in onto this set and try to understand what they tell us.
```{r}
