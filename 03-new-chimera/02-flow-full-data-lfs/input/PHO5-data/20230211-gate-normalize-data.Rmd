...
sample <- read.csv("20230211-sample-list.csv") %>% column_to_rownames(var = "file")
data.path = "/Volumes/rdss_bhe2/Project/E013-Pho4p-evolution/flow-cytometry/data/20230224-LS-chimera-production-run/PHO5-flow-data/20230211pho5-day4/"
...
```
> this sample is majority low expression. no way to rescue.

```
list.redo <- c("218-555-2")
...
final <- left_join(as_tibble(pData(fs)), stats, by = c("name" = "sample")) %>% 
  ...
  mutate(flag = case_when(
    n_induction < 1000 ~ "fail",
    sample == "221-555-1" ~ "fail",
    .default = "pass"
  ))
#final[final$sample == "221-555-1", "flag"] = "fail"
...
write_tsv(final, "20230211-gated-median-out.txt")
```
