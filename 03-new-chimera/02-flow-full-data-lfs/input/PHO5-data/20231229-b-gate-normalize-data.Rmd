---
title: "New 4/5 breakpoints in PHO5pr-RFP, day 2, plate 2"
author: Bin He, Lindsey Snyder
date: "2022-12-29 (updated `r Sys.time()`)"
output:
  html_notebook:
    toc: true
    toc_depth: 4
...
sample <- read.csv("20231229-b-sample-list.csv") %>% column_to_rownames(var = "file")
data.path = "/Volumes/rdss_bhe2/Project/E013-Pho4p-evolution/flow-cytometry/data/20231228-LS-chimeras-PHO5pr-newbreakpoint/20231229-2/"
...
ggcyto(gs[sample(length(gs), 36)], aes(x = FSC.H, y = FSC.W), subset = "-outlier") + geom_hex(bins = 128) + geom_gate("singlet") + 
  facet_wrap(~name, ncol = 6)+ scPars + 
  geom_stats(type = c("percent", "count"), location = "fixed", adjust = c(125e3, 250))
```
> 422-555 didn't grow very well
...
```
> nothing needs to be gated with a 2-cluster gate. however,
> * 416 appears to be a dead chimera
> * 420-555-2/3 didn’t grow well on both days
> * 422-555 all didn’t grow well on both days (my box highlights in the plot above was misaligned by one place)
> * 423-555-1/2 appear to be dead chimeras, while 423-555-3 appears to be functional, but didn’t grow well
> * 425-555-1/2/3 have a secondary population, and didn’t grow very well, but we can probably still use the final gated population

Implement the 2 cluster gates
```
list.redo <- c("422-555-3", "425-555-2")
...
write_tsv(final, "20231229-b-gated-median-out.txt")
```
