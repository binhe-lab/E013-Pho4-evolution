---
title: "Testing Cg aa470 helix extension, day 1"
author: Bin He, Lindsey Snyder
date: "2024-02-07 (updated `r Sys.time()`)"
output:
  html_notebook:
    toc: true
    toc_depth: 4
...
This Rmd is originally authored in the same folder as the raw data on RDSS. It will later be synced to the project git repo.
...
sample <- read.csv("20240126-sample-list.csv") %>% column_to_rownames(var = "file")
data.path = "/Volumes/rdss_bhe2/Project/E013-Pho4p-evolution/flow-cytometry/data/20240207-LS-mixed Sc45:Cg4 aa470/20240126/"
...
source("20220326-simplify-names-subroutine.R")
#require(PTXQC)
...
```
> Need to deal with NA-156-3

Implement the 2 cluster gates
```{r}
list.redo <- c("NA-156-3")
newGate <- lapply(list.redo, function(x){
  gate_flowclust_2d(dat[[x]], "BL1.H", "YL2.H", K = 2, quantile = 0.9, target = c(2,1))
})
...
write_tsv(final, "20240126-gated-median-out.txt")
```
