---
title: "Full set of chimeric Pho4 in PHO5pr-RFP, redo 2"
author: Bin He, Lindsey Snyder
date: "2022-03-31 (updated `r Sys.time()`)"
output:
  html_notebook:
    toc: true
    toc_depth: 4
...
sample <- read.csv("20230331-sample-list.csv") %>% column_to_rownames(var = "file")
data.path = "/Volumes/rdss_bhe2/Project/E013-Pho4p-evolution/flow-cytometry/data/20230224-LS-chimera-production-run/newflow-repeated some chimeras/20230331pho5pr-redo2/"
...
```{r eval=FALSE, include=FALSE}
test <- shortNames[c(49, 12+3, 12*2+3, 12*3+3)]
...
ggcyto(gs[sample(1:60, 36)], aes(x = FSC.H, y = FSC.W), subset = "-outlier") + geom_hex(bins = 128) + geom_gate("singlet") + 
...
ggcyto(gs, aes(x = BL1.H, y = YL2.H), subset = "singlet") + geom_hex(bins = 80) +
  facet_wrap(~name, ncol = 8) + scale_x_logicle() + scale_y_logicle() + panel_border()
```
...
```
> 194-555-1 and 334-555-3 had multiple populations. NA-blank-1 seems to have some cells carried over, which is likely why Lindsey didn't include this well in the list.csv (I manually added it)

Implement the 2 cluster gates
```{r}
list.redo <- c("334-555-3", "194-555-1")
...
write_tsv(final, "20230331-gated-median-out.txt")
```
