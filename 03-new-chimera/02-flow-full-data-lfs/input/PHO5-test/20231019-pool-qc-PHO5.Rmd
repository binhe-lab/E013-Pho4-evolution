...
Based on manual inspection, I identified a few more abnormal samples -- whose GFP and RFP levels are clearly different from other biological replicates, and may be non-expressing. Label them as "fail" manually and remove them from further analyses.
```{r}
fail <- tribble(
  ~date, ~name, ~reason,
  "02/16", "221-555-1", 1,
...
expt <- dat %>% 
  filter(host %in% c("373", "555"), !plasmid %in% c("188", "194")) %>% 
...
expt %>% 
  ...
  facet_grid(date ~ .) +
  scale_fill_manual(values = c("373" = "gray30", "555" = "gray70")) +
  ...
  theme(axis.text.x = element_text(angle = 90),
        legend.position = "top")
```
> pH218 has limited number of replicates after removing some of them due to low event count after gating.
...
```{r}
dat %>% #filter(flag == "pass") %>% 
  count(plasmid, host, date) %>% 
  filter(n < 3)
```
...
dat.export <- tmp %>% 
  filter(host != "156") %>% 
  mutate(host = ordered(host, levels = c("555", "373", "529"), labels = c("PHO2", "pho2∆", "PHO84")))
write_tsv(dat.export, "20231023-PHO5-bg-subtracted-data.tsv")
```

Leave out the PHO84 reporter samples
```{r}
dat1 <- filter(dat.export, host != "PHO84")
```
...
dat1 %>% filter(plasmid %in% c("188", "194", "NA")) %>% 
...
```{r}
dat.wide.fil <- filter(dat1, date != "02/10")
```
...
left_join(high.var.list, dat.wide.fil) %>% arrange(date, plasmid) %>% View()
```
