...
author: Bin He
date: "2022-07-26 (updated `r Sys.Date()`)"
output: 
  html_notebook:
    toc: true
    toc_float: true
...
# Background
This analysis builds on the 06/07 one and adds a few new chimeras.

# Goal
- Routine quality control for the new batch of repeat experiments. Determine if there is any well position effect and how well the values agree across multiple days of experiments.
- Once QC is done, merge the datasets
...
raw <- list(
  ...
  "07/26/22" = read_tsv("../data/20220726-EO-region-4.2/20220726-gated-median-out.txt", show_col_types = FALSE),
  "08/03/22" = read_tsv("../data/20220803-EO-repeat-for-yH156/20220803-gated-median-out.txt", show_col_types = FALSE)
)
...
> 07/26 and 08/03 runs both had high BL1.H readings in the yH156 strain. Check other GFP-strains

```{r}
dat %>% filter(is.na(plasmid)) %>% 
  mutate(date = gsub("/22", "", date)) %>% 
  ggplot(aes(x = date, y = BL1.H)) + geom_point() +
  stat_summary(fun.data = "mean_se", geom = "pointrange", color = "red", position = position_nudge(x = 0.1)) +
  facet_wrap(~host, scale = "free_y") +
  theme_bw(base_size = 14) + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

> How about GFP+ strains?

```{r}
dat %>% filter(plasmid %in% c("188", "194")) %>% 
  mutate(date = gsub("/22", "", date)) %>% 
  ggplot(aes(x = date, y = BL1.H)) + geom_point() +
  stat_summary(fun.data = "mean_se", geom = "pointrange", color = "red", position = position_nudge(x = 0.1)) +
  facet_wrap(~host+plasmid, scale = "free_y") +
  theme_bw(base_size = 14) + theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
