...
```{r}
selected <- as.character(
  expression(CCCSC, CCCcsC, CCCscC,
    CSSCC, CSSSC, CSScsC, CSSscC
             ))
#selected <- filter(meta, symbol %in% selected) %>% pull(plasmid)
my_plot_subset_ximera(selected) + scale_x_continuous(expand = expansion(mult = c(0.02, 0.15)))
ggsave("../img/20240310-P2ID-split-rel-activity.png", width = 3.5, height = 3.5)
```
