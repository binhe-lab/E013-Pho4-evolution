---
title: "Full set of chimeric Pho4 in PHO5pr-RFP, redo 1"
author: Bin He, Lindsey Snyder
date: "2022-03-30 (updated `r Sys.time()`)"
output:
  html_notebook:
    toc: true
    toc_depth: 4
...
sample <- read.csv("20230330-sample-list.csv") %>% column_to_rownames(var = "file")
data.path = "/Volumes/rdss_bhe2/Project/E013-Pho4p-evolution/flow-cytometry/data/20230224-LS-chimera-production-run/newflow-repeated some chimeras/20230330pho5pr-redo1/"
...
ggcyto(gs[sample(1:length(fs), 36)], aes(x = FSC.H, y = FSC.W), subset = "-outlier") + geom_hex(bins = 128) + geom_gate("singlet") + 
...
```
> 251-555-1 has some problem

Implement the 2 cluster gates
```{r}
list.redo <- c("251-555-1")
...
write_tsv(final, "20230330-gated-median-out.txt")
```
